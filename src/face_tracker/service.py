...
class TrackingService:
    ...
    def latest(self) -> dict[str, Any] | None:
        with self._lock:
            if time.monotonic() - self._published_monotonic > 0.5:
                return None
    ...
    def _run(self) -> None:
        try:
            ...
            with create_tracker(self.settings) as tracker, LatestCamera(
                self._open_camera, self._stop_event, self._set_status,
                pace_file=isinstance(self.settings.opencv_camera_source, str)
                          and Path(self.settings.camera_source).is_file(),
            ) as camera:
                while not self._stop_event.is_set():
                    ...
                    if started - sample.monotonic_s > 0.25:
                        continue
                    tracking = tracker.process(sample.image, int(sample.monotonic_s * 1000))
                    finished = time.monotonic()
                    if not camera.is_current(sample) or finished - sample.monotonic_s > 0.25:
                        continue
                    sequence += 1
                    packet = {
                        'protocol_version': '1.0', 'type': 'face_tracking', 'sequence': sequence,
                        ...
                        'queue_age_ms': round((started - sample.monotonic_s) * 1000, 2),
                        'capture_dropped_frames': camera.slot.dropped,
                        'frame': {'width': int(sample.image.shape[1]), 'height': int(sample.image.shape[0]),
                                  'fps': round(sample.fps, 2)}, **tracking,
                    }
                    with self._lock:
                        # A reconnect can clear the cache while inference builds this packet.
                        if self._stop_event.is_set() or self._status['state'] != 'running' or not camera.is_current(sample):
                            continue
                        self._latest = packet
